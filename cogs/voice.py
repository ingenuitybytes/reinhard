...
class Voice(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is not None:
            if before.channel.category_id == get_category_by_name(before.channel.guild, TEMPCATEGORY).id:
                print(f"{member.name} left a temp channel")
            for channel in self.current_channels:
                        if before.channel.name == f"{channel}-meeting".lower() and len(before.channel.members) == 0:
                            await before.channel.delete()
                            self.current_channels.remove(channel)
                            break
                                           
        if after.channel is not None:
            if after.channel.name == TEMPCHANNEL:
                channel = await create_voice_channel(after.channel.guild, f"{member.name}-meeting".lower(), category_name=TEMPCATEGORY, user_limit=after.channel.user_limit, bitrate=after.channel.bitrate)
